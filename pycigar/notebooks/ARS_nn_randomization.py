...
class ARSAgent():
    def __init__(self):
        ...
        self.num_directions = 2 #number of random directions to consider
        self.num_best_directions = 2 #number of best directions to consider
        assert self.num_best_directions <= self.num_directions
        self.max_iterations = 1 #number of iterations
        ...
        self.theta1 = np.zeros((self.n_inputs, self.n_layer1))
        self.theta2 = np.zeros((self.n_layer1, self.n_outputs))
    ...
    def rollout(self, env, theta1, theta2):
        state = env.reset()
        #rollout for episode:
        done = False
        sum_rewards = 0
    ...
    def random_search(self):
        ...
        for i in range(0,self.num_directions):
            ...
            r_pos.append(self.rollout(self.env, theta_d1_pos, theta_d2_pos))
            r_neg.append(self.rollout(self.env, theta_d1_neg, theta_d2_neg))
    ...
    def evaluation(self, theta1, theta2, iteration):
        avg_r = 0
        for i in range(8):
            r = self.rollout(self.env_eval, theta1, theta2)
            if iteration % 20 == 0:
                Logger = logger()
    ...
    def train(self):
        ...
        while k < self.max_iterations:
            #run step of ARS
            _theta1, _theta2 = self.random_search()
            thetas1.append(_theta1)
            thetas2.append(_theta2)
            if k % 1 == 0:
                avg_r = self.evaluation(_theta1, _theta2, k)
...
if __name__ == '__main__':
    ...
    sim_params['attack_randomization']['generator'] = 'DuplicateAttackGenerator'
    ...
    sim_params_eval = copy.deepcopy(sim_params)
    sim_params_eval['attack_randomization']['generator'] = 'AttackGeneratorEvaluation'
    ...
    rewards_df.to_csv(os.path.join(os.path.expanduser(args.save_path), 'ARS_pycigar_rewards.csv'))

    #save best weights to csv
    np.savetxt(os.path.join(os.path.expanduser(args.save_path), 'ARS_pycigar_theta1.csv'), thetas1[-1], delimiter=',')
    np.savetxt(os.path.join(os.path.expanduser(args.save_path), 'ARS_pycigar_theta2.csv'), thetas2[-1], delimiter=',')
