...
def save_best_policy(trainer, episodes):
    ...
    if 'best_eval_reward' not in trainer.config or trainer.config['best_eval_reward'] < mean_r:
        ...
        trainer.get_policy('agent_2').export_model(os.path.join(trainer.config['reporter_dir'], 'best', 'policy' + '_' + str(trainer.iteration), 'agent_2'))
    ...
    trainer.get_policy('agent_2').export_model(os.path.join(trainer.config['reporter_dir'], 'latest', 'policy' + '_' + str(trainer.iteration), 'agent_2'))
