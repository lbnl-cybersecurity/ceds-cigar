...
class Graph:
    ...
    def __init__(self):
        bus_name = dss.Circuit.AllBusNames()
        self.load_name = dss.Loads.AllNames()
        # No. of vertices
        self.V = len(bus_name)
        ...
        self.get_line_bus_dict()
        for k in self.line_bus_dict.keys():
            bus_idxs = self.line_bus_dict[k] 
            bus_idx1 = bus_name.index(bus_idxs[0])
            bus_idx2 = bus_name.index(bus_idxs[1])
            self.addEdge(bus_idx1, bus_idx2)

        for d in range(0, self.V):  
            self.printAllPaths(0, d)

        self.cleanPaths()

        self.convert_english()
    ...
    def get_line_bus_dict(self):
        ...
        # value = line index
        for line, line_name in enumerate(dss.Lines.AllNames()):
            dss.Lines.Name(line_name)
            b1 = dss.Lines.Bus1().split('.')[0]
            b2 = dss.Lines.Bus2().split('.')[0]
            key = (b1, b2)
            bus_line_dict[key] = line
            line_bus_dict[line_name] = [b1, b2]
        ...
        for transformer in dss.Transformers.AllNames():
            ...
            b2 = buses[1].split('.')[0]
            key = tuple([b1, b2])
            if key not in line_bus_dict:
                bus_line_dict[key] = line
                line_bus_dict[transformer] = [b1, b2]
        self.bus_line_dict = bus_line_dict
        self.line_bus_dict = line_bus_dict
    
    def get_line_lengths(self):
        line_lengths = []
        # all line lengths
        for line in self.line_bus_dict.keys():
            if line in dss.Lines.AllNames():
                dss.Lines.Name(line)
...
def whole_plot(log_dict, inverter_list, adversary_inv, network, track_colors, cutoffs, redblue, g, custom_metrics):
    ...
    def all_inv_translation_and_slope(inv_data):
        t_s_dict = {}
        for inv in inv_data:
                a_val = log_dict[inv]['control_setting']
                init_a = custom_metrics['init_control_settings'][inv]
    ...
    def make_waterfall_plot(waterfall_time, title='Waterfall Plot', x_axis_label='Distance from source (m)', y_axis_label='Voltage (V)'):
        ...
        for j in range(len(g.updatedPaths)):
            ...
            for i in range(len(g.updatedPaths[j])-1): 
                ...
                summative += g.line_lengths[g.bus_line_dict[key]]
            ...
            for ii in range(len(y)):          
                ...
                cds = ColumnDataSource(data=dict(x=path_lengths, y0=y[ii])) 

                p = wfall.line('x', 'y0',  hover_color="firebrick", source=cds, color=colors[ii], 
                              line_width=3)
                
                hover = HoverTool(renderers=[p], tooltips=[("voltage", "$y0")])
    ...
    select_node_temp = g.load_name[:2] #['s701a', 's701b'] #TODO
    select_init_node_temp = g.load_name[:2]
    ...
    node_list_selection = MultiChoice(value=select_init_node_temp, options=node_end)
...
class AppInit:
    ...
    def __init__(self, log_dict, custom_metrics, dss_file):
        print(dss_file)
        print('redirect ' + dss_file)
    ...
    def build_network(self):
        bus1 = []
        bus2 = []
        for buses in self.g.bus_line_dict.keys():
            bus1.append(buses[0])
            bus2.append(buses[1])
    ...
    def make_plot(self):  
        ...
        def total_app(doc):
            ...
            layout = whole_plot(self.log_dict, self.inverter_list, self.adversary_inv, self.network, 
                                self.track_colors, self.cutoffs, self.redblue, self.g, self.custom_metrics)
            doc.add_root(layout)
