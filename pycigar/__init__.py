...
def main(misc_inputs_path, dss_path, load_solar_path, breakpoints_path, test, policy, output):
    ...
    def save_best_policy(trainer, episodes):
        ...
        if 'best_eval_reward' not in trainer.global_vars or trainer.global_vars['best_eval_reward'] < mean_r:
            ...
            # save info
            info = {'epoch': trainer.iteration, 'reward': mean_r}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=DATA_DIR + "/ieee37busdata/misc_inputs.csv",
        help='Directory to params.csv file.',
    )
    parser.add_argument(
        "--dss", type=os.path.abspath, default=DATA_DIR + "/ieee37busdata/ieee37.dss", help='Directory to .dss file.'
    )
    parser.add_argument(
        "--loadpv",
        type=os.path.abspath,
        default=DATA_DIR + "/ieee37busdata/load_solar_data.csv",
        help='Directory to load-solar.csv file.',
    )
    parser.add_argument(
        "--breakpoints",
        type=os.path.abspath,
        default=None,
        help='Directory to custom initial breakpoints.csv file (Optional)',
    )
