...
class DelayObservation(ObservationWrapper):
    ...
    def observation_space(self):
        return Box(low=-float('inf'), high=float('inf'), shape=(6 + self.a_size + DELAY, ), dtype=np.float64)
    ...
    def observation(self, observation, info):
        if info:
            ...
            delay = info[list(info.keys())[0]]['delay']
        else:
        ...
        delay_list = np.zeros(DELAY)
        delay_list[delay] += 1
        observation = np.array([observation['y']*10, observation['u_worst']*10, p_set, *old_a_encoded, va, vb, vc, *delay_list])
