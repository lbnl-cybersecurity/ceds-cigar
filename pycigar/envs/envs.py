...
class Env8500(gym.Env):
    def __init__(self, sim_params, simulator='opendss'):
        ...
        self.device_list = sorted(df.columns)

        self.api = PyCIGAROpenDSSAPI(port=9999)
    ...
    def reset(self):
        ...
        self.api.set_slack_bus_voltage(1.0)
        max_tap_change = 16
        forward_band = 1
        tap_delay = 2
        delay = 30
...
class Device:
    def __init__(self, env):
        ...
        self.low_pass_filter_output = 0.04
...
class AttackChangeRandom:
    def __init__(self):
        ...
        start_time = np.linspace(100, 6000, 10).astype(int)
        self.scenarios = []
        for p in percentage:
            for s in start_time:
                s = int(s)
...
class AttackChange:
    def __init__(self):
        self.mode = 0
        percentage = np.linspace(10, 20, 2) / 100
        start_time = np.linspace(100, 6000, 2).astype(int)
        self.scenarios = []
        for p in percentage:
            for s in start_time:
                s = int(s)
