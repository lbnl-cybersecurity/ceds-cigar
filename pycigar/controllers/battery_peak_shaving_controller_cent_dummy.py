...
class BatteryPeakShavingControllerCentDummy(RLController):
            ...
            for device in self.device_id:
                ...
                if self.p_set[-1] >= 0:
                    ...
                    if self.p_set[-1] >= env.k.device.devices[device]['device'].max_charge_power/1e3:
                        ...
                        result[device] = (None, {'control_mode': 'charge', 'p_in': 0})
                    else:
                        result[device] = (None, {'control_mode': 'charge', 'p_in': 0})
                ...
                # discharge
                    ...
                    if self.p_set[-1] <= -env.k.device.devices[device]['device'].max_discharge_power/1e3:
                        ...
                        result[device] = (None, {'control_mode': 'discharge', 'p_out': 0})
                    else:
