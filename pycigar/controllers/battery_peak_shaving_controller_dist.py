    ...
    def get_action(self, env):
        ...
        if env.k.time == 0 or env.k.time == 51:
            ...
            for k1 in range(0,2):               
                ...
                self.measured_active_power.append(power[0])
                self.measured_reactive_power.append(power[1])
                self.measured_apparent_power.append((self.measured_active_power[-1]**2 + self.measured_reactive_power[-1]**2)**(1/2))

                self.measured_active_power_lpf.append(power[0])
                self.measured_reactive_power_lpf.append(power[1])
                self.measured_apparent_power_lpf.append((self.measured_active_power[-1]**2 + self.measured_reactive_power[-1]**2)**(1/2))

                self.load_active_power.append(power[0])
                self.load_reactive_power.append(power[1])
                self.load_apparent_power.append((self.load_active_power[-1]**2 + self.load_reactive_power[-1]**2)**(1/2))

                self.load_active_power_lpf.append(power[0])
                self.load_reactive_power_lpf.append(power[1])
            ...
            # self.p_set_temp = self.p_set[-2] + 10*self.K_P*(self.active_power_error[-1] - self.active_power_error[-2]) + self.K_I*self.Ts*self.active_power_error[-2]
            ...
            if self.p_set[-1] >= 0:
                ...
                if self.p_set[-1] >= env.k.device.devices[self.device_id]['device'].max_charge_power/1e3:
                    self.p_set[-1] = env.k.device.devices[self.device_id]['device'].max_charge_power/1e3
                ...
                if self.p_set[-1] <= -env.k.device.devices[self.device_id]['device'].max_discharge_power/1e3:
                    self.p_set[-1] = -env.k.device.devices[self.device_id]['device'].max_discharge_power/1e3
                # if self.p_set[-1] <= (self.P_target - self.measured_active_power_lpf[-2]):
                #     self.p_set[-1] = (self.P_target - self.measured_active_power_lpf[-2])
