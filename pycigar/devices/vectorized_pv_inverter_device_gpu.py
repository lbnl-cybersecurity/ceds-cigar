...
class VectorizedPVDevice:
    def __init__(self, k):
        ...
        self.Sbar = k.device.get_device(self.list_device[0]).Sbar
        ...
        for device_id in self.list_device:
            ...
            self.control_setting.append(device.control_setting)
        ...
        self.solar_generation = cp.array(self.solar_generation)
        self.control_setting = cp.array(self.control_setting)
        self.low_pass_filter_measure = cp.array(self.low_pass_filter_measure)
        self.low_pass_filter_output = cp.array(self.low_pass_filter_output)
        ...
        self.p_set = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.q_set = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.p_out = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.q_out = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.low_pass_filter_v = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.lpf_psi = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.lpf_epsilon = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.lpf_y1 = deque([cp.array([0]*len(self.list_device))]*2, maxlen=2)
        self.lpf_x = deque([cp.array([0]*len(self.list_device))]*15, maxlen=15)
        self.x = deque([[0]*len(self.list_device)]*15, maxlen=15)

        self.solar_irr = None
        self.y = None

        self.VBP = cp.zeros([len(self.list_device), 5])  # it will be update with the set_control_setting method
    ...
    def update(self, k):
        ...
        if not hasattr(self, 'step'):
            ...
            self.output_one = cp.ones([len(self.list_device),15])
        ...
        if k.time > 1:
            ...
            self.x.append(vk)
            output = cp.array(self.x).T

            vk = cp.array(vk)
            vkm1 = cp.array(vkm1)

            mask1 = cp.max(output[:, STEP_BUFFER:-STEP_BUFFER], axis=1) - cp.min(output[:, STEP_BUFFER:-STEP_BUFFER], axis=1) > 0.004
            norm_data = -1 + 2 * (output - cp.min(output, axis=1)[:, None]) / (cp.max(output, axis=1) - cp.min(output, axis=1))[:, None]
            step_corr = cp.array(signal.fftconvolve(cp.asnumpy(norm_data), self.step, mode='valid', axes=1))
            mask2 = cp.max(cp.abs(step_corr), axis=1) > 10
            mask = mask1 & mask2

            filter_data = cp.where(cp.broadcast_to(mask[:, None], (mask.shape[0], 15)), output, self.output_one)[:, STEP_BUFFER:-STEP_BUFFER]
        ...
        pk = cp.zeros(len(self.list_device))
        qk = cp.zeros(len(self.list_device))
        q_avail = cp.zeros(len(self.list_device))
        ...
        if k.time > 1:
            ...
            q_avail[idx] = -(self.Sbar**2 - pk[idx] ** 2) ** (1 / 2)
            ...
            qk[idx] = (self.Sbar**2 - pk[idx]**2)**(1/2)
    ...
    def update0(self, k):
        ...
        if not hasattr(self, 'step'):
            self.step = cp.hstack((1 * cp.ones(11), cp.linspace(1, -1, 7), -1 * cp.ones(11)))[:, None].T
            self.output_one = cp.ones([len(self.list_device),15])
...
def _filter(output, output_one, step, lpf_epsilon, lpf_psi, lpf_y1, lpf_high_pass_filter, lpf_low_pass_filter, lpf_delta_t, gain):
    mask1 = cp.max(output[:, STEP_BUFFER:-STEP_BUFFER], axis=1) - cp.min(output[:, STEP_BUFFER:-STEP_BUFFER], axis=1) > 0.004    #ok
    norm_data = -1 + 2 * (output - cp.min(output, axis=1)[:, None]) / (cp.max(output, axis=1) - cp.min(output, axis=1))[:, None] #ok
    step_corr = signal.fftconvolve(norm_data, step, mode='valid', axes=1) #ok
    mask2 = cp.max(cp.abs(step_corr), axis=1) > 10 #ok
    mask = mask1 & mask2 #ok

    filter_data = cp.where(cp.broadcast_to(mask[:, None], (mask.shape[0], 15)), output, output_one)[:, STEP_BUFFER:-STEP_BUFFER]
...
def _calculate_pq(vk, vkm1, low_pass_filter_v, solar_irr, VBP, p_set, q_set, p_out, q_out, device_num, T, lpf_o, lpf_m, solar_min_value, Sbar):
        pk = cp.zeros(device_num)
        qk = cp.zeros(device_num)
        q_avail = cp.zeros(device_num)
