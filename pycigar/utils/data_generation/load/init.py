...
from src.stochastic_computations import generate_mean_reversion_rate, generate_polynomial, euler_maruyama_method, euler_maruyama_method_new
...
class LoadGenerator:
    def __init__(self, data, input_time='15T', output_time='1S'):
        order = [int(d.split('/')[-1].split('_')[0]) for d in data]
    ...
    def generate_load(self, load_name, order):
        all_autocor = euler_maruyama_method_new(self.em_mu, self.em_x, order, self.mrr, self.output_time, self.index_vals, self.spl)
        return all_autocor