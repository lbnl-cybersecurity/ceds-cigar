...
from pycigar.controllers import FixedLineController, BaseLineController
from pycigar.controllers import FixedRegController, BaseRegController
from pycigar.controllers import RLController
from pycigar.core.kernel.device import KernelDevice
from pycigar.devices import PVDevice, RegulatorDevice, LineDevice
...
class OpenDSSDevice(KernelDevice):
    ...
    def __init__(self, master_kernel):
        ...
        self.reg_device_ids = []
        self.num_reg_devices = 0
        self.line_device_ids = []
        self.num_line_devices = 0
    ...
    def add(self, name, connect_to, device, controller, adversary_controller=None, hack=None, device_type=None):
        # print(type(device[0]))
        if isinstance(device[0], PVDevice) or device_type == 'pv_device':
            return self.add_pv_device(name, connect_to, device, controller, adversary_controller, hack)
        elif isinstance(device[0], RegulatorDevice) or device_type == 'reg_device':
            return self.add_regulator_device(name, connect_to, device, controller, adversary_controller, hack)
        elif isinstance(device[0], LineDevice) or device_type == 'line_device':
            return self.add_line_device(name, connect_to, device, controller, adversary_controller, hack)
        else:
            print('Device Type Not Implemented from kernel/device/opendss.py')
            raise NotImplementedError

    def add_line_device(self, name, connect_to, device=(LineDevice, {}), controller=(BaseLineController, {}),
                        adversary_controller=None, hack=None):
        device_id = name
        device_obj = device[0](device_id, device[1], _kernel=self.master_kernel)
        self.line_device_ids.append(device_id)
        self.num_line_devices += 1
        ...
        if adversary_controller is not None:
            ...
            adversary_device_obj = device[0](adversary_device_id, device[1], _kernel=self.master_kernel)
            ...
            self.devices[adversary_device_id] = {
                ...
                "node_id": connect_to,
                "hack_controller": FixedLineController(adversary_device_id, controller[1])
            }
        self.all_device_ids.extend((device_id, adversary_device_id))
        self.num_devices += 1
        return adversary_device_id

    def add_regulator_device(self, name, connect_to, device=(RegulatorDevice, {}), controller=(BaseRegController, {}),
                             adversary_controller=None, hack=None):
        device_id = name
        device_obj = device[0](device_id, device[1], _kernel=self.master_kernel)
        self.reg_device_ids.append(device_id)
        self.num_reg_devices += 1
        controller_obj = controller[0](device_id, additional_params=controller[1])
        self.devices[device_id] = {
            "device": device_obj,
            "controller": controller_obj,
        ...
        if adversary_controller is not None:
            ...
            self.devices[adversary_device_id] = {
                ...
                "controller": adversary_controller_obj,
                "node_id": connect_to,
                "hack_controller": FixedRegController(adversary_device_id, controller[1])
            }
        self.all_device_ids.extend((device_id, adversary_device_id))
        self.num_devices += 1
        return adversary_device_id

    def add_pv_device(self, name, connect_to, device, controller=(AdaptiveInverterController, {}),
                      adversary_controller=None, hack=None):
    ...
    def update(self, reset):
        ...
        if reset is True:
            ...
            # Update Regulator Device
            for reg_device in self.reg_device_ids:
